...
# Dung lai truoc khi ghi neu server khong dung cau hinh 2024 hoac ten/IP client da ton tai.
[ "$EUID" -eq 0 ] || { echo "Hay chay bang sudo bash"; exit 1; }
[ -f "$WG_DIR/wg0.conf" ] || { echo "Chua co wg0.conf"; exit 1; }
wg show wg0 >/dev/null || { echo "wg0 chua chay"; exit 1; }
grep -Eq '^Address[[:space:]]*=[[:space:]]*10\.7\.0\.1/24' "$WG_DIR/wg0.conf" || { echo "Mang WG khac 10.7.0.0/24: dung lai"; exit 1; }
grep -Eq '^ListenPort[[:space:]]*=[[:space:]]*51820' "$WG_DIR/wg0.conf" || { echo "Cong WG khac 51820: dung lai"; exit 1; }
for i in $(seq 1 "$NUM_CLIENTS"); do
  [ ! -e "$CLIENT_DIR/client${i}.conf" ] || { echo "Client $i da ton tai: dung lai"; exit 1; }
  ! grep -Eq "^AllowedIPs[[:space:]]*=[[:space:]]*10\\.7\\.0\\.$((i+1))/32" "$WG_DIR/wg0.conf" || { echo "IP client $i da ton tai: dung lai"; exit 1; }
done
install -d -m 700 "$CLIENT_DIR"
cp -p "$WG_DIR/wg0.conf" "$WG_DIR/wg0.conf.backup-before-batch-$(date +%Y%m%d%H%M%S)"
